...
#
#
def send_message_to_control(message, s):
...
if __name__ == "__main__":
    ...
    control_address = 'localhost' #sys.argv[1]
    ...
    # find hostname ip
    # if not control_address.isnumeric():
    #     control_address = socket.gethostbyname(control_address)
    try:
        # Check if control_address is not already an IP address
        if not control_address.isnumeric():
            resolved_address = socket.gethostbyname(control_address)
        else:
            resolved_address = control_address  # It's already an IP address
    except socket.gaierror as e:
        print(control_address)
        print(f"Error resolving hostname to IP: {e}")
        sys.exit(-1)

    # connect client to control
    s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    s.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
    s.connect((control_address, control_port))
